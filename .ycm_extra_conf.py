...
def FlagsOfCompilationDatabase(root, filename):
    try:
...
def FlagsForFile(filename):
    ...
    compilation_db_flags = FlagsOfCompilationDatabase(root, filename)
    if compilation_db_flags:
        final_flags = compilation_db_flags
    else:
        final_flags = BASE_FLAGS
        clang_flags = FlagsOfClangComplete(root)
        if clang_flags:
